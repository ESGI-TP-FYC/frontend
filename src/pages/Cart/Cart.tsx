...
const Cart: FC = () => {
    ...
    const handleInputChange = (event: ChangeEvent<HTMLInputElement>, productId: number): void => {
        if (isNaN(parseInt(event.target.value)) || parseInt(event.target.value) === 0 || parseInt(event.target.value) > 99) {
            ...
            setProductInCart(productInCart.set(productId, parseInt(event.target.value)));
            localStorage.setItem("products", JSON.stringify(Array.from(productInCart.entries())));
        }
    ...
    const onIncrease = (productId: number): void => {
        setProductInCart(productInCart.set(productId, productInCart.get(productId) + 1));
        localStorage.setItem("products", JSON.stringify(Array.from(productInCart.entries())));
        dispatch(calculateCartPrice(products));
    };
                                    ...
                                    <div key={product.id} className="card mb-3 mx-auto" style={{maxWidth: "940px"}}>
                                        <div className="row no-gutters">
                                            <div className="col-2 mx-3 my-3">
                                                <img src={product.filename} className="img-fluid"/>
                                            </div>
                                            <div className="col-6">
                                                <div className="card-body">
                                                    <h4 className="card-title">{product.productr + " " + product.productTitle}</h4>
                                            ...
                                            <div className="col-2">
                                                <div className="card-body">
                                                    <h5 className="card-title">
                                                        <span>$ {product.price * productInCart.get(product.id)}</span>
                                                    </h5>
